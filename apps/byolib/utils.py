...
def _format_dict(
    ...
    for k, v in attr.items():
        k_repr = repr(k)
        v_str = (
            _pformat(v, indent, width, curr_indent + len(k_repr)) if is_dataclass(v) else repr(v)
        )
...
def _format_list(
    ...
    list_repr = [
        _pformat(l, indent, width, curr_indent) if is_dataclass(l) else repr(l) for l in attr
    ]
