...
ExtractedTask::ExtractedTask(String task_name, IRModule mod, Optional<Target> target, Array<IRModule> dispatched) {
...
Optional<ObjectRef> MetaScheduleContext::QueryInsideWithScope(
    runtime::String task_name, IRModule mod, Optional<Target> target, Optional<Array<IRModule>> dispatched) {
...
Optional<ObjectRef> TaskExtractionNode::Query(runtime::String task_name, IRModule mod, Optional<Target> target,
...
Optional<ObjectRef> ApplyHistoryBestNode::Query(runtime::String task_name, IRModule mod, Optional<Target> target,
                                                Optional<Array<IRModule>> dispatched) {
  ICHECK(dispatched.defined());
  ICHECK_EQ(dispatched.value().size(), 1);
  ICHECK(HasOnlyOneFunction<relay::Function>(mod)) << mod;
